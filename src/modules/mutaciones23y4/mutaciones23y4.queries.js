...
async function obtenerTramitesDevueltos() {
  const query = `
    ...
    INNER JOIN ${SCHEMA2}.tbl_users_sgt_tte AS u1
      ON z.cd_func_entrega = u1.sap_user AND u1.id_role_user = 10
    INNER JOIN ${SCHEMA2}.tbl_users_sgt_tte AS u2
      ON z.cd_func_recibe = u2.sap_user AND u2.id_role_user = 11
    WHERE z.devuelto = 'X'
  `;
...
async function obtenerDevueltosPorPrediadorResumen() {
  const query = `
    ...
    INNER JOIN ${SCHEMA2}.tbl_users_sgt_tte AS u_rev
      ON z.cd_func_entrega = u_rev.sap_user AND u_rev.id_role_user = 10
    INNER JOIN ${SCHEMA2}.tbl_users_sgt_tte AS u_pred
      ON z.cd_func_recibe = u_pred.sap_user AND u_pred.id_role_user = 11
    WHERE z.devuelto = 'X'
...
async function obtenerDevueltosPorPrediadorDetalle(params = {}) {
  ...
  const countQuery = `
    ...
    INNER JOIN ${SCHEMA2}.tbl_users_sgt_tte AS u_rev
     ...
     AND u_rev.id_role_user = 10
    INNER JOIN ${SCHEMA2}.tbl_users_sgt_tte AS u_pred
      ON z.cd_func_recibe = u_pred.sap_user
     AND u_pred.id_role_user = 11
    ${whereSql}
  `;
